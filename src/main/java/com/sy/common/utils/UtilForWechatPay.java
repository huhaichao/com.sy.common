...
import com.sy.common.bean.AppPayWechatBean.MaxLength;
import com.sy.common.bean.AppPayWechatBean.Required;
import com.sy.common.bean.AppPayWechatBean.SignField;
import com.sy.common.bean.AppPayWechatBean.WechatPaySignable;
public abstract class UtilForWechatPay {
	
	  private static final String WX_DATE_FORMAT = "yyyyMMddHHmmss";
	  private static Logger logger = LoggerFactory.getLogger(UtilForWechatPay.class);
	  ...
	  public static void validateField(Object valueObject, Field field) {
	    ...
	    if (field.isAnnotationPresent(Required.class)) {
	      ...
	      if (field.getType().equals(String.class)) {
	        ...
	        if (UtilForWechatPay.isEmpty(strValue)) {
	          throw new IllegalArgumentException("Field [" + field.getName() + "] is required.");
	        }
	      }
	    }
	    ...
	    if (field.getType().equals(String.class) && field.isAnnotationPresent(MaxLength.class)) {
	      ...
	      String strValue = (String) value;
	      if (!UtilForWechatPay.isEmpty(strValue) && strValue.length() > annotatedMaxLength) {
	   ...
	   * @return
	   */
	  public static String validateFieldsAndGenerateWxSignature(WechatPaySignable valueObject,
	      String key) {
	    if (isEmpty(key)) {
	    ...
	    for (Field field : allFields) {
	      // Validate whether the field is missing value if required and etc.
	    	UtilForWechatPay.validateField(valueObject, field);
	      Object value = UtilForWechatPay.getFieldValue(valueObject, field);
	    ...
	    String md5Digest;
	    try {
	      md5Digest = UtilForWechatPay.bytesToHexString(UtilForWechatPay.md5(signInput.getBytes("UTF-8"))).toUpperCase();
	   ...
	   */
	  public static boolean validateSign(WechatPaySignable pdu, String key) {
	    logger.debug("Validating pdu " + pdu);
	    String recomputedSign = UtilForWechatPay.validateFieldsAndGenerateWxSignature(pdu, key);
